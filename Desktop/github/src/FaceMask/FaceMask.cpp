...
void FaceMask::getEvent(HttpSession *session) {
    ip = "172.16.1.98";
    urlGetData = HTTPS_URL + ip + FACEMASK + token;
    ...
    if (session->start() == true) {
        ...
        for (auto i : res["data"]["events"]) {
            ...
            urlGetImage = HTTPS_URL + ip + IMAGE_ID_FACEMASK + image_id + ACCESS_TOKEN + token;
